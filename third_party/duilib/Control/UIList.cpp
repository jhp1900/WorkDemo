...
namespace DuiLib {
  ...
  CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1), m_bMultiSelect(false)
  {
    m_pList = new CListBodyUI(this);
    m_pHeader = new CListHeaderUI;
    pSelectItemArray = new CStdValArray(sizeof(int), 32);
  ...
  void CListUI::DoEvent(TEventUI& event)
  {
    ...
    if (event.Type == UIEVENT_KILLFOCUS)
    {
      m_bFocused = false;
      if (m_bMultiSelect) {
        RECT list_rect = GetPos();
        RECT control_rect = event.pSender->GetPos();
        IListItemUI * list_item = static_cast<IListItemUI*>(event.pSender->GetInterface(_T("ListItem")));
        if (list_item && list_item->GetOwner() == this)
          return;
        if (!pSelectItemArray->IsEmpty()) {
          int item_num = pSelectItemArray->GetSize();
          CControlUI* pControl;
          for (int index = 0; index < item_num; ++index) {
            int *pIndex = static_cast<int*>(pSelectItemArray->GetAt(index));
            pControl = GetItemAt(*pIndex);
            if (pControl) {
              IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
              pListItem->Select(false);
            }
          }
          pSelectItemArray->Empty();
        }
        if (m_iCurSel >= 0) {
    ...
    if (event.Type == UIEVENT_RBUTTONDOWN) {
      if (!pSelectItemArray->IsEmpty() || m_iCurSel >= 0) {
        int item_num = pSelectItemArray->GetSize();
        if (item_num) {
          for (int index = 0; index < item_num; ++index) {
            int *pIndex = static_cast<int*>(pSelectItemArray->GetAt(index));
            CControlUI* pControl = GetItemAt(*pIndex);
            if (pControl) {
              IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
              pListItem->Select(false);
            }
  ...
  bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
  {
    ...
    if (m_pManager != NULL  && m_bMultiSelect == false) {
      m_pManager->SendNotify(this, _T("itemselect"), m_iCurSel, iOldSel);
    }
    pSelectItemArray->Add(&m_iCurSel);
  ...
  bool CListUI::GetMultiArray(int &selectIndex, CStdValArray* &pMutilArray) const {
    if (!pSelectItemArray->IsEmpty()) {
      pMutilArray = pSelectItemArray;
      return true;
    } else if (m_iCurSel >= 0) {
      selectIndex = m_iCurSel;
      return true;
    }
    return false;
  }
  bool CListUI::RemoveMultiIndex(int index) {
    return pSelectItemArray->Remove(index);
  }
  void CListUI::SetMultiArryEmpty() {
    pSelectItemArray->Empty();
  }
  void CListUI::SetCurSelectIndex(int index /* = -1 */) {
    m_iCurSel = index;
  }
